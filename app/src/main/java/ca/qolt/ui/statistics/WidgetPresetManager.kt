    ...
    suspend fun loadPreset(): List<Widget>? {
        ...
        return try {
            ...
            presetString.split(",").forEachIndexed { index, part ->
                ...
                // Return template types - actual data will be generated by ViewModel
                val widgetType: WidgetType? = when (typeId) {
                    "focus_time" -> WidgetType.FocusTime(totalHours = 0f, period = "", todayHours = 0f, trendPercentage = 0f)
                    "focus_time_today" -> WidgetType.FocusTimeToday(todayHours = 0f, vsAverage = 0f, weeklyData = emptyList(), dayLabels = emptyList())
                    "streak" -> WidgetType.Streak(currentStreak = 0, targetStreak = 30)
                    "weekly_goal" -> WidgetType.WeeklyGoal(currentProgress = 0, targetProgress = 7, percentage = 0)
                    "app_usage" -> WidgetType.AppUsage(apps = emptyList())
                    "total_hours" -> WidgetType.TotalHours(hours = 0f)
                    "stats_today" -> WidgetType.StatsToday(value = 0f, unit = "hours")
                    "monthly_overview" -> WidgetType.MonthlyOverview(totalHours = 0f, weeklyData = emptyList(), weekLabels = emptyList())
                    "focus_sessions" -> WidgetType.FocusSessions(subtitle = "", currentProgress = 0, targetProgress = 7, completedDays = emptyList(), period = "")
                    else -> null
                }
